...
bool Camera::OnUpdate(float ts) {
	...
	if (Input::IsKeyDown(KeyCode::E) && MovementSpeed < 2.0f) {
		MovementSpeed += 0.005f;
	}
	else if (Input::IsKeyDown(KeyCode::Q) && MovementSpeed > 0.0f) {
		MovementSpeed -= 0.005f;
	}
	...
	if (delta.x != 0.0f || delta.y != 0.0f)
	{
		...
		if (Input::IsKeyDown(KeyCode::LeftShift)) {
			m_Position -= rightDirection * delta.x * MovementSpeed;
			m_Position += upDirection * delta.y * MovementSpeed;

			moved = true;
		}
		// Rotation
		else {
			float pitchDelta = delta.y * MovementSpeed;
			float yawDelta = delta.x * MovementSpeed;
